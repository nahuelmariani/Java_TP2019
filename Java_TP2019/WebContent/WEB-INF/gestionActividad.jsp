...
<table border="1" cellpadding="5">
	...
	<tbody>
	<% for (Actividad inst : la) {%>
		<tr>
			<td><%=inst.getId_actividad()%></td>
			<td><%=inst.getNom_actividad()%></td>
			<td><%=inst.getDesc_actividad()%></td>
			<td><%=inst.getImporte_adicional()%></td>
		
			<td colspan="2">
				<form method="post" action="Actividades">
					<input type="hidden" name="idActividad" value="<%= inst.getId_actividad() %>">
