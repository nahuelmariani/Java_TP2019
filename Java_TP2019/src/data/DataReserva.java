...
public class DataReserva {
	...
	public void add(Reserva r) {
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn().
					prepareStatement(
							"insert into reserva(fecha_hora_desde, fecha_hora_hasta,id_instalacion,id_usuario, confirmada) values(?,?,?,?,?)",
			...
			stmt.setBoolean(5, r.getConfirmada());
			/*
	...
	public void delete(Reserva r) {
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn().
					prepareStatement("delete from reserva where id_reserva=?");
	...
	public void update(Reserva r) {
		...
		try {
			stmt=FactoryConexion.getInstancia().getConn().
					prepareStatement("update reserva set confirmada=? where id_reserva=?");
	...
	public ArrayList<Reserva> getByInstalacion(int idInstalacion) {
		...
		stmt=FactoryConexion.getInstancia().getConn().prepareStatement(
				"select r.id_reserva, i.nom_instalacion, r.fecha_reserva, r.fecha_hora_desde, r.fecha_hora_hasta, r.fecha_cancelacion from reserva r "
				+ "inner join instalacion i on i.id_instalacion=r.id_instalacion where r.id_instalacion=?"
				);
